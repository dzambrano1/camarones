...
<title>Camarones Configuracion Nitritos</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
  ...
  CONFIGURACION NITRITOS
  </h3>
</div> 
<!-- New Entry Modal Configuracion Nitritoss -->

<!-- Add New Vacuna Nitritoss Button -->
...
<div class="modal fade" id="newEntryModal" tabindex="-1" aria-labelledby="newEntryModalLabel">
      ...
      <div class="modal-content">
          <div class="modal-header">
              <h5 class="modal-title" id="newEntryModalLabel">
                  <i class="fas fa-plus-circle me-2"></i>Configurar Nuevo Nitritos
              </h5>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
              <form id="newNitritossForm">
          ...
          <div class="modal-footer btn-group">
              ...
              <button type="button" class="btn btn-success" id="saveNewNitritoss">
  ...
  <!-- DataTable for cac_nitritos records -->
...
<div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="nitritosTable" class="table table-striped table-bordered">
              ...
              <tbody>
                  <?php
                      $nitritosQuery = "SELECT * FROM cac_nitritos";

                      $stmt = $conn->prepare($nitritosQuery);
                      $stmt->execute();
                      $nitritosData = $stmt->fetchAll(PDO::FETCH_ASSOC);

                      if (empty($nitritosData)) {
                          echo "<tr><td colspan='5' class='text-center'>No hay registros disponibles</td></tr>";
                      } else {
                          foreach ($nitritosData as $row) {
                              echo "<tr>";
                              
                              // Column 0: Actions
                              echo '<td class="text-center">';
                              echo '    <div class="btn-group" role="group">';
                              echo '        <button class="btn btn-warning btn-sm edit-nitritos" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '" 
                                              data-vacuna="' . htmlspecialchars($row['cac_nitritos_vacuna'] ?? '') . '" 
                                              data-dosis="' . htmlspecialchars($row['cac_nitritos_dosis'] ?? '') . '" 
                                              data-costo="' . htmlspecialchars($row['cac_nitritos_costo'] ?? '') . '" 
                                              data-vigencia="' . htmlspecialchars($row['cac_nitritos_vigencia'] ?? '') . '"
                                              title="Editar Configuracion Vacuna Nitritoss">
                                              <i class="fas fa-edit"></i>
                                          </button>';
                              echo '        <button class="btn btn-danger btn-sm delete-nitritos" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                              title="Eliminar Configuracion Producto Nitritoss">
                              ...
                              // Column 1: Vacuna
                              echo "<td>" . htmlspecialchars($row['cac_nitritos_vacuna'] ?? '') . "</td>";
                              
                              // Column 2: Dosis
                              echo "<td>" . htmlspecialchars($row['cac_nitritos_dosis'] ?? 'N/A') . "</td>";
                              
                              // Column 3: Costo
                              echo "<td>" . htmlspecialchars($row['cac_nitritos_costo'] ?? 'N/A') . "</td>";
                              
                              // Column 4: Vigencia
                              echo "<td>" . htmlspecialchars($row['cac_nitritos_vigencia'] ?? 'N/A') . "</td>";
...
<!-- Initialize DataTable for VH nitritos -->
<script>
$(document).ready(function() {
    $('#nitritosTable').DataTable({
...
$(document).ready(function() {
    ...
    // Note: editNitritossModal is created dynamically later, so no need to initialize here.

    // Handle new entry form submission
    $('#saveNewNitritoss').click(function() {
        // Validate the form
        var form = document.getElementById('newNitritossForm');
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: 'process_configuracion_nitritos.php',
                    type: 'POST',
                    data: {
                        action: 'insert',
                    ...
                    success: function(response) {
                        ...
                        Swal.fire({
                            ...
                            text: 'El registro de nitritos ha sido guardado correctamente',
                            icon: 'success',
                            confirmButtonColor: '#28a745'
                        }).then(() => {
    ...
    $('.edit-nitritos').click(function() {
        ...
        // Edit Configuracion Nitritoss Modal dialog for editing
        var modalHtml = `
        <div class="modal fade" id="editNitritossModal" tabindex="-1" aria-labelledby="editNitritossModalLabel">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editNitritossModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Producto Control Nitritos
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form id="editNitritossForm">
                    ...
                    <div class="modal-footer btn-group">
                        ...
                        <button type="button" class="btn btn-success" id="saveEditNitritoss">
        ...
        $('#editNitritossModal').remove();
        
        // Add the modal to the page
        $('body').append(modalHtml);
        
        // Show the modal
        var editModal = new bootstrap.Modal(document.getElementById('editNitritossModal'));
        editModal.show();
        
        // Handle save button click
        $('#saveEditNitritoss').click(function() {
            // Create a form object to properly validate
            var form = document.getElementById('editNitritossForm');
            if (!form.checkValidity()) {
                form.reportValidity();
                return;
            }
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar la configuracion de nitritos?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    $('.delete-nitritos').click(function() {
        var id = $(this).data('id');
        
        // Confirm before deleting using SweetAlert2
        Swal.fire({
            title: '¿Eliminar registro?',
            text: `¿Está seguro de que desea eliminar la configuracion de nitritos? Esta acción no se puede deshacer.`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc3545',
    ...
    $(document).on('click', '.register-new-nitritos-btn', function() { 
