...
<title>Camarones Configuracion Oxigeno</title>
<!-- Link to the Favicon -->
...
<div class="modal fade" id="newEntryModal" tabindex="-1" aria-labelledby="newEntryModalLabel">
      ...
      <div class="modal-content">
          <div class="modal-header">
              <h5 class="modal-title" id="newEntryModalLabel">
                  <i class="fas fa-plus-circle me-2"></i>Configurar Nuevo Producto Control Oxigeno
              </h5>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
              <form id="newOxigenoForm">
          ...
          <div class="modal-footer btn-group">
              ...
              <button type="button" class="btn btn-success" id="saveNewOxigeno">
  ...
  <!-- DataTable for cac_oxygen records -->  
<div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="oxigenoTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
              ...
              <tbody>
                  <?php
                      $oxigenoQuery = "SELECT * FROM cac_oxygen";

                      $stmt = $conn->prepare($oxigenoQuery);
                      $stmt->execute();
                      $oxigenosData = $stmt->fetchAll(PDO::FETCH_ASSOC);

                      if (empty($oxigenosData)) {
                          echo "<tr><td colspan='5' class='text-center'>No hay registros disponibles</td></tr>";
                      } else {
                          foreach ($oxigenosData as $row) {
                              echo "<tr>";
                              
                              // Column 0: Actions
                              echo '<td class="text-center">';
                              echo '    <div class="btn-group" role="group">';
                              echo '        <button class="btn btn-warning btn-sm edit-oxigeno" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '" 
                                              data-vacuna="' . htmlspecialchars($row['cac_oxygen_vacuna'] ?? '') . '" 
                                              data-dosis="' . htmlspecialchars($row['cac_oxygen_dosis'] ?? '') . '" 
                                              data-costo="' . htmlspecialchars($row['cac_oxygen_costo'] ?? '') . '" 
                                              data-vigencia="' . htmlspecialchars($row['cac_oxygen_vigencia'] ?? '') . '"
                                              title="Editar Configuracion Vacuna Oxigeno">
                                              <i class="fas fa-edit"></i>
                                          </button>';
                              echo '        <button class="btn btn-danger btn-sm delete-oxigeno" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                              title="Eliminar Configuracion Producto Oxigeno">
                              ...
                              // Column 1: Vacuna
                              echo "<td>" . htmlspecialchars($row['cac_oxygen_vacuna'] ?? '') . "</td>";
                              
                              // Column 2: Dosis
                              echo "<td>" . htmlspecialchars($row['cac_oxygen_dosis'] ?? 'N/A') . "</td>";
                              
                              // Column 3: Costo
                              echo "<td>" . htmlspecialchars($row['cac_oxygen_costo'] ?? 'N/A') . "</td>";
                              
                              // Column 4: Vigencia
                              echo "<td>" . htmlspecialchars($row['cac_oxygen_vigencia'] ?? 'N/A') . "</td>";
...
<!-- Initialize DataTable for VH oxigeno -->
<script>
$(document).ready(function() {
    $('#oxigenoTable').DataTable({
...
$(document).ready(function() {
    ...
    // Note: editOxigenoModal is created dynamically later, so no need to initialize here.

    // Handle new entry form submission
    $('#saveNewOxigeno').click(function() {
        // Validate the form
        var form = document.getElementById('newOxigenoForm');
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: 'process_configuracion_oxigeno.php',
                    type: 'POST',
                    data: {
                        action: 'insert',
                    ...
                    success: function(response) {
                        ...
                        Swal.fire({
                            ...
                            text: 'El registro de oxigeno ha sido guardado correctamente',
                            icon: 'success',
                            confirmButtonColor: '#28a745'
                        }).then(() => {
    ...
    $('.edit-oxigeno').click(function() {
        ...
        // Edit Configuracion Oxigeno Modal dialog for editing
        var modalHtml = `
        <div class="modal fade" id="editOxigenoModal" tabindex="-1" aria-labelledby="editOxigenoModalLabel">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editOxigenoModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Producto Control Oxigeno
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form id="editOxigenoForm">
                    ...
                    <div class="modal-footer btn-group">
                        ...
                        <button type="button" class="btn btn-success" id="saveEditOxigeno">
        ...
        $('#editOxigenoModal').remove();
        
        // Add the modal to the page
        $('body').append(modalHtml);
        
        // Show the modal
        var editModal = new bootstrap.Modal(document.getElementById('editOxigenoModal'));
        editModal.show();
        
        // Handle save button click
        $('#saveEditOxigeno').click(function() {
            // Create a form object to properly validate
            var form = document.getElementById('editOxigenoForm');
            if (!form.checkValidity()) {
                form.reportValidity();
                return;
            }
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar la configuracion de oxigeno?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    $('.delete-oxigeno').click(function() {
        var id = $(this).data('id');
        
        // Confirm before deleting using SweetAlert2
        Swal.fire({
            title: '¿Eliminar registro?',
            text: `¿Está seguro de que desea eliminar la configuracion de oxigeno? Esta acción no se puede deshacer.`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc3545',
    ...
    $(document).on('click', '.register-new-oxigeno-btn', function() { 
