...
<title>Camarones Configuracion Biofungicidas</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
  ...
  CONFIGURACION BIOFUNGICIDAS
  </h3>
</div> 
<!-- New Entry Modal Configuracion Biofungicidas -->

<!-- Add New Vacuna Biofungicidas Button -->
...
<div class="modal fade" id="newEntryModal" tabindex="-1" aria-labelledby="newEntryModalLabel">
      ...
      <div class="modal-content">
          <div class="modal-header">
              <h5 class="modal-title" id="newEntryModalLabel">
                  <i class="fas fa-plus-circle me-2"></i>Configurar Nuevo Biofungicida
              </h5>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
              <form id="newBiofungicidasForm">
          ...
          <div class="modal-footer btn-group">
              ...
              <button type="button" class="btn btn-success" id="saveNewBiofungicidas">
  ...
  <!-- DataTable for cac_biofungicida records -->
  
<div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="biofungicidasTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
              ...
              <tbody>
                  <?php
                      $biofungicidasQuery = "SELECT * FROM cac_biofungicida";

                      $stmt = $conn->prepare($biofungicidasQuery);
                      $stmt->execute();
                      $biofungicidassData = $stmt->fetchAll(PDO::FETCH_ASSOC);

                      if (empty($biofungicidassData)) {
                          echo "<tr><td colspan='5' class='text-center'>No hay registros disponibles</td></tr>";
                      } else {
                          foreach ($biofungicidassData as $row) {
                              echo "<tr>";
                              
                              // Column 0: Actions
                              echo '<td class="text-center">';
                              echo '    <div class="btn-group" role="group">';
                              echo '        <button class="btn btn-warning btn-sm edit-biofungicidas" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '" 
                                              data-vacuna="' . htmlspecialchars($row['cac_biofungicida_vacuna'] ?? '') . '" 
                                              data-dosis="' . htmlspecialchars($row['cac_biofungicida_dosis'] ?? '') . '" 
                                              data-costo="' . htmlspecialchars($row['cac_biofungicida_costo'] ?? '') . '" 
                                              data-vigencia="' . htmlspecialchars($row['cac_biofungicida_vigencia'] ?? '') . '"
                                              title="Editar Configuracion Vacuna Biofungicidas">
                                              <i class="fas fa-edit"></i>
                                          </button>';
                              echo '        <button class="btn btn-danger btn-sm delete-biofungicidas" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                              title="Eliminar Configuracion Producto Biofungicidas">
                              ...
                              // Column 1: Vacuna
                              echo "<td>" . htmlspecialchars($row['cac_biofungicida_vacuna'] ?? '') . "</td>";
                              
                              // Column 2: Dosis
                              echo "<td>" . htmlspecialchars($row['cac_biofungicida_dosis'] ?? 'N/A') . "</td>";
                              
                              // Column 3: Costo
                              echo "<td>" . htmlspecialchars($row['cac_biofungicida_costo'] ?? 'N/A') . "</td>";
                              
                              // Column 4: Vigencia
                              echo "<td>" . htmlspecialchars($row['cac_biofungicida_vigencia'] ?? 'N/A') . "</td>";
...
<!-- Initialize DataTable for VH biofungicidas -->
<script>
$(document).ready(function() {
    $('#biofungicidasTable').DataTable({
...
$(document).ready(function() {
    ...
    // Note: editBiofungicidasModal is created dynamically later, so no need to initialize here.

    // Handle new entry form submission
    $('#saveNewBiofungicidas').click(function() {
        // Validate the form
        var form = document.getElementById('newBiofungicidasForm');
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: 'process_configuracion_biofungicidas.php',
                    type: 'POST',
                    data: {
                        action: 'insert',
                    ...
                    success: function(response) {
                        ...
                        Swal.fire({
                            ...
                            text: 'El registro de biofungicidas ha sido guardado correctamente',
                            icon: 'success',
                            confirmButtonColor: '#28a745'
                        }).then(() => {
    ...
    $('.edit-biofungicidas').click(function() {
        var id = $(this).data('id');
        ...
        // Edit Configuracion Biofungicidas Modal dialog for editing
        var modalHtml = `
        <div class="modal fade" id="editBiofungicidasModal" tabindex="-1" aria-labelledby="editBiofungicidasModalLabel">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editBiofungicidasModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Biofungicida
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form id="editBiofungicidasForm">
                    ...
                    <div class="modal-footer btn-group">
                        ...
                        <button type="button" class="btn btn-success" id="saveEditBiofungicidas">
        ...
        $('#editBiofungicidasModal').remove();
        
        // Add the modal to the page
        $('body').append(modalHtml);
        
        // Show the modal
        var editModal = new bootstrap.Modal(document.getElementById('editBiofungicidasModal'));
        editModal.show();
        
        // Handle save button click
        $('#saveEditBiofungicidas').click(function() {
            // Create a form object to properly validate
            var form = document.getElementById('editBiofungicidasForm');
            if (!form.checkValidity()) {
                form.reportValidity();
                return;
            }
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar la configuracion de biofungicidas?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    $('.delete-biofungicidas').click(function() {
        var id = $(this).data('id');
        
        // Confirm before deleting using SweetAlert2
        Swal.fire({
            title: '¿Eliminar registro?',
            text: `¿Está seguro de que desea eliminar la configuracion de biofungicidas? Esta acción no se puede deshacer.`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc3545',
    ...
    $(document).on('click', '.register-new-biofungicidas-btn', function() { 
