...
<title>Camarones Configuracion Redox</title>
<!-- Link to the Favicon -->
...
<!-- New Entry Modal Configuracion Redox -->

<!-- Add New Vacuna Redox Button -->
...
<div class="modal fade" id="newEntryModal" tabindex="-1" aria-labelledby="newEntryModalLabel">
      ...
      <div class="modal-content">
          <div class="modal-header">
              <h5 class="modal-title" id="newEntryModalLabel">
                  <i class="fas fa-plus-circle me-2"></i>Configurar Nuevo Producto Control Redox
              </h5>
  ...
  <!-- DataTable for cac_redox records -->
  
<div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="redoxsTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
              ...
              <tbody>
                  <?php
                      $redoxQuery = "SELECT * FROM cac_redox";

                      $stmt = $conn->prepare($redoxQuery);
                      $stmt->execute();
                      $redoxData = $stmt->fetchAll(PDO::FETCH_ASSOC);

                      if (empty($redoxData)) {
                          echo "<tr><td colspan='5' class='text-center'>No hay registros disponibles</td></tr>";
                      } else {
                          foreach ($redoxData as $row) {
                              echo "<tr>";
                              
                              // Column 0: Actions
                              echo '<td class="text-center">';
                              echo '    <div class="btn-group" role="group">';
                              echo '        <button class="btn btn-warning btn-sm edit-redox" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '" 
                                              data-vacuna="' . htmlspecialchars($row['cac_redox_vacuna'] ?? '') . '" 
                                              data-dosis="' . htmlspecialchars($row['cac_redox_dosis'] ?? '') . '" 
                                              data-costo="' . htmlspecialchars($row['cac_redox_costo'] ?? '') . '" 
                                              data-vigencia="' . htmlspecialchars($row['cac_redox_vigencia'] ?? '') . '"
                                              title="Editar Configuracion Vacuna Redox">
                                              <i class="fas fa-edit"></i>
                                          </button>';
                              echo '        <button class="btn btn-danger btn-sm delete-redox" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                              title="Eliminar Configuracion Producto Redox">
                              ...
                              // Column 1: Vacuna
                              echo "<td class='text-center'>" . htmlspecialchars($row['cac_redox_vacuna'] ?? '') . "</td>";
                              
                              // Column 2: Dosis
                              echo "<td class='text-center'>" . htmlspecialchars($row['cac_redox_dosis'] ?? 'N/A') . "</td>";
                              
                              // Column 3: Costo
                              echo "<td class='text-center'>" . htmlspecialchars($row['cac_redox_costo'] ?? 'N/A') . "</td>";
                              
                              // Column 4: Vigencia
                              echo "<td class='text-center'>" . htmlspecialchars($row['cac_redox_vigencia'] ?? 'N/A') . "</td>";
...
<!-- Initialize DataTable for VH redox -->
<script>
$(document).ready(function() {
    $('#redoxTable').DataTable({
...
$(document).ready(function() {
    ...
    $('#saveNewAmoniacos').click(function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: 'process_configuracion_redox.php',
                    type: 'POST',
                    data: {
                        action: 'insert',
                    ...
                    success: function(response) {
                        ...
                        Swal.fire({
                            ...
                            text: 'El registro de redox ha sido guardado correctamente',
                            icon: 'success',
                            confirmButtonColor: '#28a745'
                        }).then(() => {
    ...
    $('.edit-redox').click(function() {
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editAmoniacosModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Producto Control Redox
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
        ...
        $('#saveEditAmoniacos').click(function() {
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar la configuracion de redox?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    $('.delete-redox').click(function() {
        var id = $(this).data('id');
        
        // Confirm before deleting using SweetAlert2
        Swal.fire({
            title: '¿Eliminar registro?',
            text: `¿Está seguro de que desea eliminar la configuracion de redox? Esta acción no se puede deshacer.`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc3545',
    ...
    $(document).on('click', '.register-new-redox-btn', function() { 
