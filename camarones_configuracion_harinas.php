...
<title>Camarones PLAN ALIMENTO HARINAS</title>
<!-- Link to the Favicon -->
                ...
                <div class="col-md-4 mb-3">
                    <div class="card border-info h-100">
                        <div class="card-body p-3">
                            ...
                            <p class="card-text small text-muted">
                                Mezcla de harinas de soya, trigo, arroz, pescado, y subproductos vegetales según disponibilidad local.
                            </p>
                        </div>
                    </div>
                </div>
...
<div class="container text-center">
  ...
  CONFIGURACION HARINAS CAMARONES
  </h3>
</div> 
...
<div class="modal fade" id="newEntryModal" tabindex="-1" aria-labelledby="newEntryModalLabel">
      ...
      <div class="modal-content">
          <div class="modal-header">
              <h5 class="modal-title" id="newEntryModalLabel">
                  <i class="fas fa-plus-circle me-2"></i>Configurar Nuevo Harina Camarones
              </h5>
                  ...
                  <div class="mb-4">
                      <div class="input-group">
                          <span class="input-group-text">
                              ...
                              <label for="new_harinas" class="form-label">Harina</label>
                            <input type="text" class="form-control" id="new_harinas" name="harinas" required>
                          </span>
                      </div>
                  </div>
                  <div class="mb-4">
  ...
  <!-- DataTable for cac_harinas records -->
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="harinasTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
              ...
              <tbody>
                  <?php
                      $harinasQuery = "SELECT * FROM cac_harinas";

                      $stmt = $conn->prepare($harinasQuery);
                      $stmt->execute();
                      $harinassData = $stmt->fetchAll(PDO::FETCH_ASSOC);

                                             if (empty($harinassData)) {
                           echo "<tr><td colspan='5' class='text-center'>No hay registros disponibles</td></tr>";
                       } else {
                          foreach ($harinassData as $row) {
                              echo "<tr>";
                              
                              // Column 0: Actions
                              echo '<td class="text-center">';
                              echo '    <div class="btn-group" role="group">';
                              echo '        <button class="btn btn-warning btn-sm edit-harinas" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '" 
                                              data-harinas="' . htmlspecialchars($row['cac_harinas_nombre'] ?? '') . '" 
                                              data-etapa="' . htmlspecialchars($row['cac_harinas_etapa'] ?? '') . '"
                                              data-costo="' . htmlspecialchars($row['cac_harinas_costo'] ?? '') . '" 
                                              data-vigencia="' . htmlspecialchars($row['cac_harinas_vigencia'] ?? '') . '"
                                              title="Editar Configuracion Vacuna Concentrado">
                                              <i class="fas fa-edit"></i>
                                          </button>';
                              echo '        <button class="btn btn-danger btn-sm delete-harinas" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                              ...
                              // Column 1: Vacuna
                              echo "<td>" . htmlspecialchars($row['cac_harinas_nombre'] ?? '') . "</td>";
                              // Columna 2: Etapa
                              echo "<td>" . htmlspecialchars($row['cac_harinas_etapa'] ?? '') . "</td>";
                              // Column 4: Costo
                              echo "<td>" . htmlspecialchars($row['cac_harinas_costo'] ?? 'N/A') . "</td>";
                              // Column 5: Vigencia
                              echo "<td>" . htmlspecialchars($row['cac_harinas_vigencia'] ?? 'N/A') . "</td>";
                              echo "</tr>";
                          }
                      }
                  ?>
...
<!-- Initialize DataTable for harinasTable -->
<script>
$(document).ready(function() {
    $('#harinasTable').DataTable({
...
$(document).ready(function() {
    ...
    $('#saveNewConcentrado').click(function() {
        ...
        Swal.fire({
            ...
            text: `¿Desea registrar el alimento ${formData.harinas} ?`,
            icon: 'question',
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: 'process_configuracion_harinas.php',
                    type: 'POST',
                    data: {
                        action: 'create',
                        harinas: formData.harinas,
                    ...
                    success: function(response) {
                        ...
                        Swal.fire({
                            ...
                            text: 'El registro de harinas ha sido guardado correctamente',
                            icon: 'success',
                            confirmButtonColor: '#28a745'
                        }).then(() => {
    ...
    $('.edit-harinas').click(function() {
        var id = $(this).data('id');
        var harinas = $(this).data('harinas');
                            ...
                            <div class="mb-2">                            
                                <div class="input-group">
                                    <span class="input-group-text">
                                        ...
                                        <label for="edit_harinas" class="form-label">Alimento</label>                                    
                                        <select class="form-select" id="edit_harinas" name="harinas" required>
                                            <option value="">Seleccionar</option>
                                            <?php
                                            // Fetch distinct names from the database
                                            $sql_names = "SELECT DISTINCT cac_harinas_nombre FROM cac_harinas ORDER BY cac_harinas_nombre ASC";
                                            $stmt_names = $conn->prepare($sql_names);
                                            $stmt_names->execute();
                                            $names = $stmt_names->fetchAll(PDO::FETCH_ASSOC);
                                            foreach ($names as $name_row) {
                                                echo '<option value="' . htmlspecialchars($name_row['cac_harinas_nombre']) . '">' . htmlspecialchars($name_row['cac_harinas_nombre']) . '</option>';
                                            }
                                            ?>
                                        </select>
                                    </span>                                    
        ...
        $('#saveEditConcentrado').click(function() {
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar la configuracion de harinas?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    $('.delete-harinas').click(function() {
        var id = $(this).data('id');
        
        // Confirm before deleting using SweetAlert2
        Swal.fire({
            title: '¿Eliminar registro?',
            text: `¿Está seguro de que desea eliminar la configuracion de harinas? Esta acción no se puede deshacer.`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc3545',
    ...
    $(document).on('click', '.register-new-harinas-btn', function() { 
...
$(document).ready(function() {
    ...
    function calculatePezROI() {
        ...
        const harinasConsumido = alimentoTotalConsumido;
        ...
        const racionDiariaConcentrado = duracionDias > 0 ? (harinasConsumido / duracionDias) : 0;
        
        // Calculate break-even point for feed cost
        const costoTotalCompra = pesoInicial * precioKgInicial;
        const ingresoVenta = pesoFinal * precioKgFinal;
        const margenDisponible = ingresoVenta - costoTotalCompra;
        const precioAlimentoEquilibrio = harinasConsumido > 0 ? (margenDisponible / harinasConsumido) : 0;
        
        // Perform financial calculations (all feed is concentrate for pigs)
        const costoTotalAlimento = harinasConsumido * costoAlimentoKg;
        const costoTotal = costoTotalCompra + costoTotalAlimento;
        ...
        const resultsHtml = `
            <div class="calculation-steps">
                ...
                <div class="step-item mb-3">
                    ...
                    <div class="step-calculation">
                        <code>harinas_total = alimento_total (100% harinas para peces)</code>
                        <div class="step-result">
                            <strong>Concentrado Total:</strong> ${formatNumber(harinasConsumido)} kg
                        </div>
                    </div>
                </div>
                ...
                <div class="step-item mb-4 border border-success rounded p-3" style="background: linear-gradient(135deg, #d4edda 0%, #c3e6cb 100%);">
                    ...
                    <div class="mt-2">
                        <small style="color: #155724; font-weight: 500;">
                            💡 Los peces requieren alimentación 100% con harinas balanceado para un ${tipoPez === 'Camarones' ? 'crecimiento óptimo' : 'rendimiento productivo óptimo'}.
                        </small>
                    </div>
                </div>
                ...
                <div class="step-item mb-3">
                    ...
                    <div class="step-calculation">
                        <code>precio_equilibrio = (ingreso_venta - costo_compra) ÷ harinas_consumido</code>
                        <div class="step-result">
                            (${formatCurrency(ingresoVenta)} - ${formatCurrency(costoTotalCompra)}) ÷ ${formatNumber(harinasConsumido)} kg = <strong class="${precioAlimentoEquilibrio > costoAlimentoKg ? 'text-success' : 'text-danger'}">${formatCurrency(precioAlimentoEquilibrio)}/kg</strong>
                        </div>
                        <small class="text-muted">
                            ${precioAlimentoEquilibrio > costoAlimentoKg ? 
                                '✅ El precio actual del harinas está por debajo del punto de equilibrio' : 
                                '⚠️ El precio actual del harinas supera el punto de equilibrio'}
                        </small>
                    </div>
                </div>
                ...
                <div class="step-item mb-3">
                    ...
                    <div class="step-calculation">
                        <code>costo_total_harinas = harinas_consumido × costo_harinas_kg</code>
                        <div class="step-result">
                            ${formatNumber(harinasConsumido)} kg × ${formatCurrency(costoAlimentoKg)}/kg = <strong>${formatCurrency(costoTotalAlimento)}</strong>
                        </div>
                    </div>
                </div>
                ...
                <div class="step-item mb-3">
                    ...
                    <div class="step-calculation">
                        <code>costo_total = costo_total_compra + costo_total_harinas</code>
                        <div class="step-result">
                ...
                <div class="summary-section">
                    ...
                    <div class="mt-3">
                        ...
                        <small class="text-muted">
                            ...
                            En peces, el harinas representa el 100% de la alimentación y es el principal costo de producción.
                        </small>
                    </div>
                </div>
            </div>
