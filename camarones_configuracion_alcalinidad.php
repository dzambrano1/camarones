...
<title>Camarones Configuracion Alcalinidad</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
  ...
  CONFIGURACION ALCALINIDAD
  </h3>
</div> 
<!-- New Entry Modal Configuracion Alcalinidads -->

<!-- Add New Vacuna Alcalinidads Button -->
...
<div class="modal fade" id="newEntryModal" tabindex="-1" aria-labelledby="newEntryModalLabel">
      ...
      <div class="modal-content">
          <div class="modal-header">
              <h5 class="modal-title" id="newEntryModalLabel">
                  <i class="fas fa-plus-circle me-2"></i>Configurar Nuevo Alcalinidad
              </h5>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
              <form id="newAlcalinidadsForm">
          ...
          <div class="modal-footer btn-group">
              ...
              <button type="button" class="btn btn-success" id="saveNewAlcalinidads">
  ...
  <!-- DataTable for cac_alcalinidad records -->
...
<div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="alcalinidadTable" class="table table-striped table-bordered">
              ...
              <tbody>
                  <?php
                      $alcalinidadQuery = "SELECT * FROM cac_alcalinidad";

                      $stmt = $conn->prepare($alcalinidadQuery);
                      $stmt->execute();
                      $alcalinidadData = $stmt->fetchAll(PDO::FETCH_ASSOC);

                      if (empty($alcalinidadData)) {
                          echo "<tr><td colspan='5' class='text-center'>No hay registros disponibles</td></tr>";
                      } else {
                          foreach ($alcalinidadData as $row) {
                              echo "<tr>";
                              
                              // Column 0: Actions
                              echo '<td class="text-center">';
                              echo '    <div class="btn-group" role="group">';
                              echo '        <button class="btn btn-warning btn-sm edit-alcalinidad" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '" 
                                              data-vacuna="' . htmlspecialchars($row['cac_alcalinidad_vacuna'] ?? '') . '" 
                                              data-dosis="' . htmlspecialchars($row['cac_alcalinidad_dosis'] ?? '') . '" 
                                              data-costo="' . htmlspecialchars($row['cac_alcalinidad_costo'] ?? '') . '" 
                                              data-vigencia="' . htmlspecialchars($row['cac_alcalinidad_vigencia'] ?? '') . '"
                                              title="Editar Configuracion Vacuna Alcalinidads">
                                              <i class="fas fa-edit"></i>
                                          </button>';
                              echo '        <button class="btn btn-danger btn-sm delete-alcalinidad" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                              title="Eliminar Configuracion Producto Alcalinidads">
                              ...
                              // Column 1: Vacuna
                              echo "<td>" . htmlspecialchars($row['cac_alcalinidad_vacuna'] ?? '') . "</td>";
                              
                              // Column 2: Dosis
                              echo "<td>" . htmlspecialchars($row['cac_alcalinidad_dosis'] ?? 'N/A') . "</td>";
                              
                              // Column 3: Costo
                              echo "<td>" . htmlspecialchars($row['cac_alcalinidad_costo'] ?? 'N/A') . "</td>";
                              
                              // Column 4: Vigencia
                              echo "<td>" . htmlspecialchars($row['cac_alcalinidad_vigencia'] ?? 'N/A') . "</td>";
...
<!-- Initialize DataTable for VH alcalinidad -->
<script>
$(document).ready(function() {
    $('#alcalinidadTable').DataTable({
...
$(document).ready(function() {
    ...
    // Note: editAlcalinidadsModal is created dynamically later, so no need to initialize here.

    // Handle new entry form submission
    $('#saveNewAlcalinidads').click(function() {
        // Validate the form
        var form = document.getElementById('newAlcalinidadsForm');
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: 'process_configuracion_alcalinidad.php',
                    type: 'POST',
                    data: {
                        action: 'insert',
                    ...
                    success: function(response) {
                        ...
                        Swal.fire({
                            ...
                            text: 'El registro de alcalinidad ha sido guardado correctamente',
                            icon: 'success',
                            confirmButtonColor: '#28a745'
                        }).then(() => {
    ...
    $('.edit-alcalinidad').click(function() {
        var id = $(this).data('id');
        ...
        // Edit Configuracion Alcalinidads Modal dialog for editing
        var modalHtml = `
        <div class="modal fade" id="editAlcalinidadsModal" tabindex="-1" aria-labelledby="editAlcalinidadsModalLabel">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editAlcalinidadsModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Producto Control Alcalinidad
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form id="editAlcalinidadsForm">
                    ...
                    <div class="modal-footer btn-group">
                        ...
                        <button type="button" class="btn btn-success" id="saveEditAlcalinidads">
        ...
        $('#editAlcalinidadsModal').remove();
        
        // Add the modal to the page
        $('body').append(modalHtml);
        
        // Show the modal
        var editModal = new bootstrap.Modal(document.getElementById('editAlcalinidadsModal'));
        editModal.show();
        
        // Handle save button click
        $('#saveEditAlcalinidads').click(function() {
            // Create a form object to properly validate
            var form = document.getElementById('editAlcalinidadsForm');
            if (!form.checkValidity()) {
                form.reportValidity();
                return;
            }
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar la configuracion de alcalinidad?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    $('.delete-alcalinidad').click(function() {
        var id = $(this).data('id');
        
        // Confirm before deleting using SweetAlert2
        Swal.fire({
            title: '¿Eliminar registro?',
            text: `¿Está seguro de que desea eliminar la configuracion de alcalinidad? Esta acción no se puede deshacer.`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc3545',
    ...
    $(document).on('click', '.register-new-alcalinidad-btn', function() { 
