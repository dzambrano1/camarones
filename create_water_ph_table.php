...
try {
    ...
    $checkTableQuery = "SHOW TABLES LIKE 'water_ph_log'";
    ...
    if ($stmt->rowCount() > 0) {
        echo json_encode([
            'success' => true,
            'message' => 'La tabla water_ph_log ya existe'
        ]);
        exit;
    }
    ...
    // Create the table
    $createTableQuery = "
    CREATE TABLE IF NOT EXISTS `water_ph_log` (
    ...
    echo json_encode([
        'success' => true,
        'message' => 'Tabla water_ph_log creada exitosamente'
    ]);
