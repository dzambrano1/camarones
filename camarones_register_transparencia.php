...
// Fetch product options from cac_transparencia table
$productOptions = [];
try {
    $sql = "SELECT id, cac_transparencia_vacuna, cac_transparencia_dosis, cac_transparencia_costo FROM cac_transparencia ORDER BY cac_transparencia_vacuna ASC";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="newEntryModalLabel">
                    <i class="fas fa-plus-circle me-2"></i>Nuevo Registro Transparencia
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="newTransparencyForm">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-field">
                                <label for="new_transparency_cm">Nivel transparencia (cm)</label>
                                <div class="field-container">
                                    <i class="fas fa-water field-icon"></i>
                                    <input type="number" step="0.01" min="0" id="new_transparency_cm" name="transparency_cm" placeholder="Ej: 15.50" required>
                                </div>
                            </div>
                            <div class="form-field">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-field">
                                <label for="new_product">Producto Transparencia</label>
                                <div class="field-container">
                                    <i class="fas fa-syringe field-icon"></i>
                                    <select id="new_product" name="product" required>
                                        <option value="">Productos</option>
                                        <?php foreach ($productOptions as $product): ?>
                                            <option value="<?php echo htmlspecialchars($product['cac_transparencia_vacuna']); ?>">
                                                <?php echo htmlspecialchars($product['cac_transparencia_vacuna']); ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
            ...
            <div class="modal-footer btn-group">
                ...
                <button type="button" class="btn btn-success" id="saveNewTransparency">
...
<div class="container mt-4 text-center">
    <button type="button" class="btn btn-success btn-lg" data-bs-toggle="modal" data-bs-target="#newEntryModal">
        <i class="fas fa-plus me-2"></i>Nuevo Registro de Transparencia
    </button>
  </div>
  
<!-- Simple DataTable for Water Transparency Records -->
<div class="container mt-5 mb-5">
    <div class="card shadow">
        <div class="card-header bg-info text-white">
            <h5 class="mb-0 text-center"><i class="fas fa-table me-2"></i>Registros de Transparencia</h5>
        </div>
        <div class="card-body">
            <div class="table-responsive">
                <table id="transparencyTable" class="table table-striped table-bordered">
                    <thead class="table-dark">
                        <tr>
                            <th class="text-center">ID Estanque</th>
                            <th class="text-center">Transparencia (cm)</th>
...
<div class="modal fade modern-modal" id="editTransparencyModal" tabindex="-1" aria-labelledby="editTransparencyModalLabel">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editTransparencyModalLabel">
                    <i class="fas fa-edit me-2"></i>Editar Registros Transparencia
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="editTransparencyForm">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-field">
                                <label for="edit_transparency_cm">Nivel Transparencia (cm)</label>
                                <div class="field-container">
                                    <i class="fas fa-tint field-icon"></i>
                                    <input type="number" step="0.01" min="0" id="edit_transparency_cm" name="transparency_cm" placeholder="Ej: 15.50" required>
                                </div>
                            </div>
                            <div class="form-field">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-field">
                                <label for="edit_product">Producto Transparencia</label>
                                <div class="field-container">
                                    <i class="fas fa-syringe field-icon"></i>
            ...
            <div class="modal-footer btn-group">
                ...
                <button type="button" class="btn btn-success" id="saveEditTransparency">
                    <i class="fas fa-save me-1"></i>Guardar
                </button>
            </div>
        </div>
...
<!-- Water Transparency Statistics -->
<div class="container mt-5 mb-5">
    <div class="row">
                <div class="col-md-4">
            <div class="card shadow">
                <div class="card-header bg-info text-white">
                    <h6 class="mb-0"><i class="fas fa-tint me-2"></i>Transparencia Promedio</h6>
                </div>
                <div class="card-body text-center">
                    <h4 id="avgTransparency">-- cm</h4>
                    </div>
                    </div>
                    </div>
        <div class="col-md-4">
            <div class="card shadow">
                <div class="card-header bg-success text-white">
                    <h6 class="mb-0"><i class="fas fa-arrow-up me-2"></i>Transparencia Máxima</h6>
                </div>
                <div class="card-body text-center">
                    <h4 id="maxTransparency">-- cm</h4>
                    </div>
                    </div>
                     </div>
        <div class="col-md-4">
            <div class="card shadow">
                <div class="card-header bg-warning text-white">
                    <h6 class="mb-0"><i class="fas fa-arrow-down me-2"></i>Transparencia Mínima</h6>
                </div>
                <div class="card-body text-center">
                    <h4 id="minTransparency">-- cm</h4>
                    </div>
            </div>
        </div>
    </div>
...
$(document).ready(function() {
    ...
    function loadStatistics() {
        $.ajax({
            url: 'get_water_transparencia_data.php',
            type: 'GET',
            dataType: 'json',
            success: function(response) {
                if (!response.success || !response.records || response.records.length === 0) {
                    $('#avgTransparency').text('-- cm');
                    $('#maxTransparency').text('-- cm');
                    $('#minTransparency').text('-- cm');
                    $('#minTransparency').text('-- cm');
                    return;
                }

                // Calculate statistics from transparency_cm column
                const transparencyLevels = response.records
                    .map(item => parseFloat(item.transparency_cm))
                    .filter(value => !isNaN(value) && value > 0); // Filter out invalid values
                
                if (transparencyLevels.length === 0) {
                    $('#avgTransparency').text('-- cm');
                    $('#maxTransparency').text('-- cm');
                    $('#minTransparency').text('-- cm');
                    $('#minTransparency').text('-- cm');
                ...
                const avgTransparency = transparencyLevels.reduce((a, b) => a + b, 0) / transparencyLevels.length;
                const maxTransparency = Math.max(...transparencyLevels);
                const minTransparency = Math.min(...transparencyLevels);
                
                // Update display with proper formatting
                $('#avgTransparency').text(avgTransparency.toFixed(2) + ' cm');
                $('#maxTransparency').text(maxTransparency.toFixed(2) + ' cm');
                $('#minTransparency').text(minTransparency.toFixed(2) + ' cm');
            },
            error: function(xhr, status, error) {
                console.error('Error loading statistics:', error);
                $('#avgTransparency').text('Error');
                $('#maxTransparency').text('Error');
                $('#minTransparency').text('Error');
            }
        });
    }
    ...
    if (typeof transparencyDataTable !== 'undefined') {
        transparencyDataTable.on('xhr.dt', function() {
            loadStatistics();
        });
    }
});
</script>
<!-- Monthly transparency Trends Chart -->
<div class="container mt-5 mb-5">
    <div class="card shadow">
        <div class="card-header bg-primary text-white">
            <h5 class="mb-0 text-center">
                <i class="fas fa-chart-line me-2"></i>Tendencias de transparencia Mensual por Estanque
            </h5>
        </div>
        <div class="card-body">
            <!-- Filter Controls -->
            ...
            <div class="row">
                <div class="col-12">
                    <div class="chart-container" style="position: relative; height: 500px; width: 100%;">
                        <canvas id="transparencyChart" style="width: 100%; height: 100%;"></canvas>
                    </div>
                </div>
            </div>
            ...
            <div class="row mt-3">
                <div class="col-md-12">
                    <small class="text-muted">
                        ...
                        Este gráfico muestra los niveles de transparencia promedio mensuales para cada estanque.
...
<script>
// transparency Chart Variables
let transparencyChart = null;
let transparencyChartData = {};

// Initialize transparency Chart
function initaltransparencyChart() {
    ...
    transparencyChart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                title: {
                    display: true,
                    text: 'Transparencia Promedio Mensual por Estanque',
                    font: {
                        size: 16,
                        weight: 'bold'
                    }
                ...
                tooltip: {
                    ...
                    callbacks: {
                        ...
                        label: function(context) {
                            ...
                            if (value !== null) {
                                return datasetLabel + ': ' + value.toFixed(2) + 'cm';
                            }
    ...
    setTimeout(() => {
        transparencyChart.resize();
    }, 100);
}

// Load transparency Chart Data
    function loadtransparencyData(tagidFilter = 'all') {
    const url = `get_monthly_transparencia_data.php${tagidFilter !== 'all' ? '?tagid=' + encodeURIComponent(tagidFilter) : ''}`;
    ...
    fetch(url)
        ...
        .then(data => {
            if (data.success) {
                ...
                transparencyChartData = data;
                
                // Update chart
                transparencyChart.data.labels = data.labels;
                transparencyChart.data.datasets = data.datasets;
                ...
                console.log('transparency chart updated successfully');
            } else {
                console.error('Error loading transparency data:', data.message);
                Swal.fire({
                    title: 'Error',
                    text: 'Error al cargar datos de transparencia: ' + data.message,
                    icon: 'error',
        ...
        .catch(error => {
            ...
            Swal.fire({
                title: 'Error',
                text: 'Error al obtener datos de transparencia',
                icon: 'error',
                confirmButtonColor: '#dc3545'
            });
        });
...
// Initialize transparency chart when page loads
$(document).ready(function() {
    // Initialize chart
    initaltransparencyChart();
    ...
    $('#pond-filter').on('change', function() {
        ...
        loadtransparencyData(selectedPond);
    });
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('saveNewTransparency').addEventListener('click', saveNewRecord);
    document.getElementById('saveEditTransparency').addEventListener('click', saveEditRecord);
});

// Initialize DataTable (CRUD))
function initializeDataTable() {
    transparencyDataTable = $('#transparencyTable').DataTable({
        ajax: {
            url: 'get_water_transparencia_data.php',
            type: 'GET',
        ...
        buttons: [
            ...
            },
            {
                ...
                filename: 'transparency_' + new Date().toISOString().split('T')[0]
            },
            {
                extend: 'excel',
    ...
    $('#transparencyTable_filter input').attr('placeholder', 'Buscar en todos los campos...');
...
function editRecord(id) {
    ...
    const rowData = transparencyDataTable.rows().data().toArray().find(r => r.id == id);
    if (!rowData) return;
    ...
    document.getElementById('edit_transparency_cm').value = rowData.transparency_cm;
    ...
    // Show modal
    const modal = new bootstrap.Modal(document.getElementById('editTransparencyModal'));
    modal.show();
}
...
function deleteRecord(id, pondId) {
    Swal.fire({
        ...
        fetch('process_water_transparencia.php', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (data.success) {
            ...
            transparencyDataTable.ajax.reload(); // Reload DataTable
...
function saveNewRecord() {
    const formData = {
        ...
        transparency_cm: document.getElementById('new_transparency_cm').value,
    ...
    .then(data => {
        if (data.success) {
            ...
            document.getElementById('newTransparencyForm').reset();
...
function saveEditRecord() {
    const formData = {
        ...
        transparency_cm: document.getElementById('edit_transparency_cm').value,
    ...
    .then(data => {
        if (data.success) {
            ...
            const modal = bootstrap.Modal.getInstance(document.getElementById('editTransparencyModal'));
            modal.hide();
            transparencyDataTable.ajax.reload(); // Reload DataTable
            $(document).trigger('tableUpdated'); // Trigger statistics update
        } else {
