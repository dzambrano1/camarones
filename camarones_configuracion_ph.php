...
<title>Camarones Configuracion pH</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
  ...
  CONFIGURACION PRODUCTOS CONTROL pH
  </h3>
</div> 
<!-- New Entry Modal Configuracion pH -->
<!-- Add New Producto Control pH Button -->
      ...
      <div class="modal-content">
          <div class="modal-header">
              <h5 class="modal-title" id="newEntryModalLabel">
                  <i class="fas fa-plus-circle me-2"></i>Configurar Nuevo Producto Control pH
              </h5>
...
<div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="pHTable" class="table table-striped table-bordered">
              ...
              <tbody>
                  <?php
                      $pHQuery = "SELECT * FROM cac_pH";

                      $stmt = $conn->prepare($pHQuery);
                      $stmt->execute();
                      $pHsData = $stmt->fetchAll(PDO::FETCH_ASSOC);

                      if (empty($pHsData)) {
                          echo "<tr><td colspan='5' class='text-center'>No hay registros disponibles</td></tr>";
                      } else {
                          foreach ($pHsData as $row) {
                              echo "<tr>";
                              
                              // Column 0: Actions
                              echo '<td class="text-center">';
                              echo '    <div class="btn-group" role="group">';
                              echo '        <button class="btn btn-warning btn-sm edit-pH" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '" 
                                              data-vacuna="' . htmlspecialchars($row['cac_ph_vacuna'] ?? '') . '" 
                                              data-dosis="' . htmlspecialchars($row['cac_ph_dosis'] ?? '') . '" 
                                              data-costo="' . htmlspecialchars($row['cac_ph_costo'] ?? '') . '" 
                                              data-vigencia="' . htmlspecialchars($row['cac_ph_vigencia'] ?? '') . '"
                                              title="Editar Configuracion Vacuna PH">
                                              <i class="fas fa-edit"></i>
                                          </button>';
                              echo '        <button class="btn btn-danger btn-sm delete-pH" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                              title="Eliminar Configuracion Tratamiento pH">
                              ...
                              // Column 1: Vacuna
                              echo "<td>" . htmlspecialchars($row['cac_ph_vacuna'] ?? '') . "</td>";
                              
                              // Column 2: Dosis
                              echo "<td>" . htmlspecialchars($row['cac_ph_dosis'] ?? 'N/A') . "</td>";
                              
                              // Column 3: Costo
                              echo "<td>" . htmlspecialchars($row['cac_ph_costo'] ?? 'N/A') . "</td>";
                              
                              // Column 4: Vigencia
                              echo "<td>" . htmlspecialchars($row['cac_ph_vigencia'] ?? 'N/A') . "</td>";
...
<!-- Initialize DataTable for VH pH -->
<script>
...
$(document).ready(function() {
    ...
    // Note: editPHModal is created dynamically later, so no need to initialize here.
    ...
    $('#saveNewPH').click(function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: 'process_configuracion_pH.php',
                    type: 'POST',
                    data: {
                        action: 'insert',
                    ...
                    success: function(response) {
                        ...
                        Swal.fire({
                            ...
                            text: 'El registro de pH ha sido guardado correctamente',
                            icon: 'success',
                            confirmButtonColor: '#28a745'
                        }).then(() => {
    ...
    $('.edit-pH').click(function() {
        ...
        // Edit Configuracion PH Modal dialog for editing
        var modalHtml = `
        <div class="modal fade" id="editPHModal" tabindex="-1" aria-labelledby="editPHModalLabel">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editPHModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Producto Control pH
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
        ...
        // Show the modal
        var editModal = new bootstrap.Modal(document.getElementById('editPHModal'));
        editModal.show();
        ...
        $('#saveEditPH').click(function() {
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar la configuracion de pH?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    $('.delete-pH').click(function() {
        ...
        Swal.fire({
            ...
            text: `¿Está seguro de que desea eliminar la configuracion de pH? Esta acción no se puede deshacer.`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc3545',
    ...
    $(document).on('click', '.register-new-pH-btn', function() { 
