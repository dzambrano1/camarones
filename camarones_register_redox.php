...
// Fetch product options from cac_redox table
$productOptions = [];
try {
    $sql = "SELECT id, cac_redox_vacuna, cac_redox_dosis, cac_redox_costo FROM cac_redox ORDER BY cac_redox_vacuna ASC";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
...
<title>Camarones Register Redox</title>
<!-- Link to the Favicon -->
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="newEntryModalLabel">
                    <i class="fas fa-plus-circle me-2"></i>Nuevo Registro Redox
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="newRedoxForm">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-field">
                                <label for="new_redox_mv">Nivel Redox (mV)</label>
                                <div class="field-container">
                                    <i class="fas fa-water field-icon"></i>
                                    <input type="number" step="0.01" min="0" id="new_redox_mv" name="redox_mv" placeholder="Ej: 15.50" required>
                                </div>
                            </div>
                            <div class="form-field">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-field">
                                <label for="new_product">Producto redox</label>
                                <div class="field-container">
                                    <i class="fas fa-syringe field-icon"></i>
                                    <select id="new_product" name="product" required>
                                        <option value="">Productos</option>
                                        <?php foreach ($productOptions as $product): ?>
                                            <option value="<?php echo htmlspecialchars($product['cac_redox_vacuna']); ?>">
                                                <?php echo htmlspecialchars($product['cac_redox_vacuna']); ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
            ...
            <div class="modal-footer btn-group">
                ...
                <button type="button" class="btn btn-success" id="saveNewRedox">
...
<div class="container mt-4 text-center">
    <button type="button" class="btn btn-success btn-lg" data-bs-toggle="modal" data-bs-target="#newEntryModal">
        <i class="fas fa-plus me-2"></i>Nuevo Registro de redox
    </button>
  </div>
  
<!-- Simple DataTable for Water Redox Records -->
<div class="container mt-5 mb-5">
    <div class="card shadow">
        <div class="card-header bg-info text-white">
            <h5 class="mb-0 text-center"><i class="fas fa-table me-2"></i>Registros de Redox</h5>
        </div>
        <div class="card-body">
            <div class="table-responsive">
                <table id="redoxTable" class="table table-striped table-bordered">
                    <thead class="table-dark">
                        <tr>
                            <th class="text-center">ID Estanque</th>
                            <th class="text-center">Redox (mV)</th>
...
<!-- Modal for Edit Water Redox -->
<div class="modal fade modern-modal" id="editRedoxModal" tabindex="-1" aria-labelledby="editRedoxModalLabel">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editRedoxModalLabel">
                    <i class="fas fa-edit me-2"></i>Editar Registros Redox
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="editRedoxForm">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-field">
                                <label for="edit_redox_mv">Nivel Redox (mV)</label>
                                <div class="field-container">
                                    <i class="fas fa-tint field-icon"></i>
                                    <input type="number" step="0.01" min="0" id="edit_redox_mv" name="redox_mv" placeholder="Ej: 15.50" required>
                                </div>
                            </div>
                            <div class="form-field">
                                 ...
                                 <div class="field-container">
                                     <i class="fas fa-weight-scale field-icon"></i>
                                     <input type="number" step="0.01" min="0" id="edit_product_qty" name="product_qty" placeholder="Litros">
                                 </div>
                             </div>
                        </div>
                        <!-- Right Column -->
                        <div class="col-md-6">
                            ...
                            <div class="form-field">
                                <label for="edit_product">Producto Redox</label>
                                <div class="field-container">
                                    <i class="fas fa-syringe field-icon"></i>
            ...
            <div class="modal-footer btn-group">
                ...
                <button type="button" class="btn btn-success" id="saveEditRedox">
                    <i class="fas fa-save me-1"></i>Guardar
                </button>
            </div>
        </div>
...
<!-- Water Redox Statistics -->
<div class="container mt-5 mb-5">
    <div class="row">
                <div class="col-md-4">
            <div class="card shadow">
                <div class="card-header bg-info text-white">
                    <h6 class="mb-0"><i class="fas fa-tint me-2"></i>Redox Promedio (mV)</h6>
                </div>
                <div class="card-body text-center">
                    <h4 id="avgRedox">-- mV</h4>
                    </div>
                    </div>
                    </div>
        <div class="col-md-4">
            <div class="card shadow">
                <div class="card-header bg-success text-white">
                    <h6 class="mb-0"><i class="fas fa-arrow-up me-2"></i>Redox Máxima (mV)</h6>
                </div>
                <div class="card-body text-center">
                    <h4 id="maxRedox">-- mV</h4>
                    </div>
                    </div>
                     </div>
        <div class="col-md-4">
            <div class="card shadow">
                <div class="card-header bg-warning text-white">
                    <h6 class="mb-0"><i class="fas fa-arrow-down me-2"></i>Redox Mínima (mV)</h6>
                </div>
                <div class="card-body text-center">
                    <h4 id="minRedox">-- mV</h4>
                    </div>
            </div>
        </div>
    </div>
...
$(document).ready(function() {
    // Load redox statistics
    function loadStatistics() {
        $.ajax({
            url: 'get_water_redox_data.php',
            type: 'GET',
            dataType: 'json',
            success: function(response) {
                ...
                // Calculate statistics from redox_mv column
                const redoxLevels = response.records
                    .map(item => parseFloat(item.redox_mv))
                    .filter(value => !isNaN(value) && value > 0); // Filter out invalid values
                
                if (redoxLevels.length === 0) {
                    $('#avgRedox').text('-- mV');
                    $('#maxRedox').text('-- mV');
                    $('#minRedox').text('-- mV');
                    $('#minRedox').text('-- mV');
                ...
                const avgRedox = redoxLevels.reduce((a, b) => a + b, 0) / redoxLevels.length;
                const maxRedox = Math.max(...redoxLevels);
                const minRedox = Math.min(...redoxLevels);
                
                // Update display with proper formatting
                $('#avgRedox').text(avgRedox.toFixed(2) + ' mV');
                $('#maxRedox').text(maxRedox.toFixed(2) + ' mV');
                $('#minRedox').text(minRedox.toFixed(2) + ' mV');
            },
    ...
    if (typeof redoxDataTable !== 'undefined') {
        redoxDataTable.on('xhr.dt', function() {
            loadStatistics();
        });
    }
});
</script>
<!-- Monthly redox Trends Chart -->
<div class="container mt-5 mb-5">
    <div class="card shadow">
        <div class="card-header bg-primary text-white">
            <h5 class="mb-0 text-center">
                <i class="fas fa-chart-line me-2"></i>Tendencias de Redox Mensual por Estanque
            </h5>
        </div>
        <div class="card-body">
            <!-- Filter Controls -->
            ...
            <div class="row">
                <div class="col-12">
                    <div class="chart-container" style="position: relative; height: 500px; width: 100%;">
                        <canvas id="redoxChart" style="width: 100%; height: 100%;"></canvas>
                    </div>
                </div>
            </div>
            ...
            <div class="row mt-3">
                <div class="col-md-12">
                    <small class="text-muted">
                        ...
                        Este gráfico muestra los niveles de redox promedio mensuales para cada estanque.
...
<script>
// redox Chart Variables
let redoxChart = null;
let redoxChartData = {};

// Initialize redox Chart
function initalredoxChart() {
    const ctx = document.getElementById('redoxChart').getContext('2d');
    
    redoxChart = new Chart(ctx, {
        type: 'line',
        data: {
            labels: [],
            datasets: []
        ...
        options: {
            ...
            plugins: {
                title: {
                    display: true,
                    text: 'Redox Promedio Mensual por Estanque',
                    font: {
                        size: 16,
                        weight: 'bold'
                    }
                ...
                tooltip: {
                    ...
                    callbacks: {
                        ...
                        label: function(context) {
                            ...
                            if (value !== null) {
                                return datasetLabel + ': ' + value.toFixed(2) + 'mV';
                            }
...
// Load redox Chart Data
    function loadredoxData(tagidFilter = 'all') {
    const url = `get_monthly_redox_data.php${tagidFilter !== 'all' ? '?tagid=' + encodeURIComponent(tagidFilter) : ''}`;
    ...
    fetch(url)
        ...
        .then(data => {
            if (data.success) {
                ...
                // Update chart
                redoxChart.data.labels = data.labels;
                redoxChart.data.datasets = data.datasets;
                redoxChart.update();
                ...
                console.log('redox chart updated successfully');
            } else {
                console.error('Error loading redox data:', data.message);
                Swal.fire({
                    title: 'Error',
                    text: 'Error al cargar datos de redox: ' + data.message,
                    icon: 'error',
        ...
        .catch(error => {
            console.error('Error fetching redox data:', error);
            Swal.fire({
                title: 'Error',
                text: 'Error al obtener datos de redox',
                icon: 'error',
                confirmButtonColor: '#dc3545'
            });
        });
...
// Initialize redox chart when page loads
$(document).ready(function() {
    // Initialize chart
    initalredoxChart();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('saveNewRedox').addEventListener('click', saveNewRecord);
    document.getElementById('saveEditRedox').addEventListener('click', saveEditRecord);
});

// Initialize DataTable (CRUD))
function initializeDataTable() {
    redoxDataTable = $('#redoxTable').DataTable({
        ajax: {
            url: 'get_water_redox_data.php',
            type: 'GET',
        ...
        columns: [
            ...
            { 
                ...
                render: function(data) {
                    return parseFloat(data).toFixed(2) + ' mV';
                }
            },
            {
                data: 'source', 
        ...
        buttons: [
            ...
            },
            {
                ...
                filename: 'redox_' + new Date().toISOString().split('T')[0]
            },
            {
                extend: 'excel',
    ...
    $('#redoxTable_filter input').attr('placeholder', 'Buscar en todos los campos...');
...
function editRecord(id) {
    ...
    const rowData = redoxDataTable.rows().data().toArray().find(r => r.id == id);
    if (!rowData) return;
    ...
    document.getElementById('edit_redox_mv').value = rowData.redox_mv;
    ...
    // Show modal
    const modal = new bootstrap.Modal(document.getElementById('editRedoxModal'));
    modal.show();
}
...
function deleteRecord(id, pondId) {
    ...
    .then(data => {
        if (data.success) {
            ...
            redoxDataTable.ajax.reload(); // Reload DataTable
...
function saveNewRecord() {
    const formData = {
        ...
        redox_mv: document.getElementById('new_redox_mv').value,
    ...
    .then(data => {
        if (data.success) {
            ...
            document.getElementById('newRedoxForm').reset();
...
function saveEditRecord() {
    const formData = {
        ...
        redox_mv: document.getElementById('edit_redox_mv').value,
    ...
    .then(data => {
        if (data.success) {
            ...
            const modal = bootstrap.Modal.getInstance(document.getElementById('editRedoxModal'));
            modal.hide();
            redoxDataTable.ajax.reload(); // Reload DataTable
            $(document).trigger('tableUpdated'); // Trigger statistics update
        } else {
