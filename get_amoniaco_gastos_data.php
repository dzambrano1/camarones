...
try {
    ...
    while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
            ...
            $months_data[$monthName] = [
                ...
                'avg_ammonia_mg_l' => round((float)$row['avg_ammonia_mg_l'], 2)
            ];
    ...
    $colors = [
        ...
        'ammonia_mg_l' => ['rgba(34, 139, 34, 0.8)', '#228B22']  // Crimson for amoniaco levels (line)
    ];
    ...
    // Create dataset for amoniaco levels (line chart on secondary y-axis)
    $ammonia_mg_l_data = [];
    foreach ($all_months as $month) {
        $value = isset($months_data[$month]) ? 
                 $months_data[$month]['avg_ammonia_mg_l'] : null;
        $ammonia_mg_l_data[] = $value;
    }
    
    $datasets[] = [
        'label' => 'Niveles de Amoniacos (mg/L)',
        'data' => $ammonia_mg_l_data,
        'type' => 'line',
        'backgroundColor' => $colors['ammonia_mg_l'][0],
        'borderColor' => $colors['ammonia_mg_l'][1],
        'borderWidth' => 3,
        'fill' => false,
        'tension' => 0.4,
        'pointBackgroundColor' => $colors['ammonia_mg_l'][1],
    ...
    foreach ($all_months as $index => $month) {
        ...
        if (isset($months_data[$month])) {
            ...
            $month_tooltip['expenses_and_amoniaco'] = [
                ...
                'avg_ammonia_mg_l' => $data['avg_ammonia_mg_l'] . ' mg/L'
            ];
        } else {
            $month_tooltip['expenses_and_amoniaco'] = [
                'total_expenses' => '$0.00',
