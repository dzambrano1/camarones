...
<title>Camarones Configuracion Salinidad</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
  ...
  CONFIGURACION SALINIDAD
  </h3>
</div> 
<!-- New Entry Modal Configuracion Salinidad -->

<!-- Add New Vacuna Salinidad Button -->
...
<div class="modal fade" id="newEntryModal" tabindex="-1" aria-labelledby="newEntryModalLabel">
      ...
      <div class="modal-content">
          <div class="modal-header">
              <h5 class="modal-title" id="newEntryModalLabel">
                  <i class="fas fa-plus-circle me-2"></i>Configurar Nuevo Producto Control Salinidad
              </h5>
  ...
  <!-- DataTable for cac_salinidad records -->
  
<div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="salinidadTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
              ...
              <tbody>
                  <?php
                      $salinidadQuery = "SELECT * FROM cac_salinidad";

                      $stmt = $conn->prepare($salinidadQuery);
                      $stmt->execute();
                      $salinidadsData = $stmt->fetchAll(PDO::FETCH_ASSOC);

                      if (empty($salinidadsData)) {
                          echo "<tr><td colspan='5' class='text-center'>No hay registros disponibles</td></tr>";
                      } else {
                          foreach ($salinidadsData as $row) {
                              echo "<tr>";
                              
                              // Column 0: Actions
                              echo '<td class="text-center">';
                              echo '    <div class="btn-group" role="group">';
                              echo '        <button class="btn btn-warning btn-sm edit-salinidad" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '" 
                                              data-vacuna="' . htmlspecialchars($row['cac_salinidad_vacuna'] ?? '') . '" 
                                              data-dosis="' . htmlspecialchars($row['cac_salinidad_dosis'] ?? '') . '" 
                                              data-costo="' . htmlspecialchars($row['cac_salinidad_costo'] ?? '') . '" 
                                              data-vigencia="' . htmlspecialchars($row['cac_salinidad_vigencia'] ?? '') . '"
                                              title="Editar Configuracion Vacuna Salinidad">
                                              <i class="fas fa-edit"></i>
                                          </button>';
                              echo '        <button class="btn btn-danger btn-sm delete-salinidad" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                              title="Eliminar Configuracion Producto Salinidad">
                              ...
                              // Column 1: Vacuna
                              echo "<td class='text-center'>" . htmlspecialchars($row['cac_salinidad_vacuna'] ?? '') . "</td>";
                              
                              // Column 2: Dosis
                              echo "<td class='text-center'>" . htmlspecialchars($row['cac_salinidad_dosis'] ?? 'N/A') . "</td>";
                              
                              // Column 3: Costo
                              echo "<td class='text-center'>" . htmlspecialchars($row['cac_salinidad_costo'] ?? 'N/A') . "</td>";
                              
                              // Column 4: Vigencia
                              echo "<td class='text-center'>" . htmlspecialchars($row['cac_salinidad_vigencia'] ?? 'N/A') . "</td>";
...
<!-- Initialize DataTable for VH salinidad -->
<script>
$(document).ready(function() {
    $('#salinidadTable').DataTable({
...
$(document).ready(function() {
    ...
    // Note: editSalinidadModal is created dynamically later, so no need to initialize here.

    // Handle new entry form submission
    $('#saveNewSalinidad').click(function() {
        // Validate the form
        var form = document.getElementById('newSalinidadForm');
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: 'process_configuracion_salinidad.php',
                    type: 'POST',
                    data: {
                        action: 'insert',
                    ...
                    success: function(response) {
                        ...
                        Swal.fire({
                            ...
                            text: 'El registro de salinidad ha sido guardado correctamente',
                            icon: 'success',
                            confirmButtonColor: '#28a745'
                        }).then(() => {
    ...
    $('.edit-salinidad').click(function() {
        ...
        // Edit Configuracion Salinidad Modal dialog for editing
        var modalHtml = `
        <div class="modal fade" id="editSalinidadModal" tabindex="-1" aria-labelledby="editSalinidadModalLabel">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editSalinidadModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Salinidad
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
        ...
        $('#editSalinidadModal').remove();
        
        // Add the modal to the page
        $('body').append(modalHtml);
        
        // Show the modal
        var editModal = new bootstrap.Modal(document.getElementById('editSalinidadModal'));
        editModal.show();
        
        // Handle save button click
        $('#saveEditSalinidad').click(function() {
            // Create a form object to properly validate
            var form = document.getElementById('editSalinidadForm');
            if (!form.checkValidity()) {
                form.reportValidity();
                return;
            }
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar la configuracion de salinidad?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    $('.delete-salinidad').click(function() {
        var id = $(this).data('id');
        
        // Confirm before deleting using SweetAlert2
        Swal.fire({
            title: '¿Eliminar registro?',
            text: `¿Está seguro de que desea eliminar la configuracion de salinidad? Esta acción no se puede deshacer.`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc3545',
    ...
    $(document).on('click', '.register-new-salinidad-btn', function() { 
