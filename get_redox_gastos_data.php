...
try {
    ...
    while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
            ...
            $months_data[$monthName] = [
                ...
                'avg_redox_mv' => round((float)$row['avg_redox_mv'], 2)
            ];
    ...
    $colors = [
        ...
        'redox_mv' => ['rgba(34, 139, 34, 0.8)', '#228B22']  // Crimson for redox levels (line)
    ];
    ...
    // Create dataset for redox levels (line chart on secondary y-axis)
    $redox_mv_data = [];
    foreach ($all_months as $month) {
        $value = isset($months_data[$month]) ? 
                 $months_data[$month]['avg_redox_mv'] : null;
        $redox_mv_data[] = $value;
    }
    
    $datasets[] = [
        'label' => 'Niveles de Redox (mV)',
        'data' => $redox_mv_data,
        'type' => 'line',
        'backgroundColor' => $colors['redox_mv'][0],
        'borderColor' => $colors['redox_mv'][1],
        'borderWidth' => 3,
        'fill' => false,
        'tension' => 0.4,
        'pointBackgroundColor' => $colors['redox_mv'][1],
    ...
    foreach ($all_months as $index => $month) {
        ...
        if (isset($months_data[$month])) {
            ...
            $month_tooltip['expenses_and_redox'] = [
                ...
                'avg_redox_mv' => $data['avg_redox_mv'] . ' mV'
            ];
        } else {
            $month_tooltip['expenses_and_redox'] = [
                'total_expenses' => '$0.00',
