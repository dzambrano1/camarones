...
try {
    ...
    // Get monthly alcalinidad expenses and alcalinidad levels
    $sql = "SELECT 
                DATE_FORMAT(timestamp, '%Y-%m') as month,
                SUM(product_qty * product_price) as total_expenses,
                AVG(product_qty * product_price) as avg_expense_per_treatment,
                COUNT(*) as treatment_count,
                AVG(alkalinity_mg_l) as avg_alkalinity_mg_l,
                MIN(alkalinity_mg_l) as min_alkalinity_mg_l,
                MAX(alkalinity_mg_l) as max_alkalinity_mg_l
            FROM water_alkalinity_log
            WHERE timestamp IS NOT NULL 
                AND product_qty IS NOT NULL 
                AND product_price IS NOT NULL
                AND alkalinity_mg_l IS NOT NULL
    ...
    while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
            ...
            $months_data[$monthName] = [
                ...
                'avg_alkalinity_mg_l' => round((float)$row['avg_alkalinity_mg_l'], 2)
            ];
    ...
    $colors = [
        ...
        'alkalinity_mg_l' => ['rgba(34, 139, 34, 0.8)', '#228B22']  // Crimson for alcalinidad levels (line)
    ];
    ...
    // Create dataset for alcalinidad levels (line chart on secondary y-axis)
    $alkalinity_mg_l_data = [];
    foreach ($all_months as $month) {
        $value = isset($months_data[$month]) ? 
                 $months_data[$month]['avg_alkalinity_mg_l'] : null;
        $alkalinity_mg_l_data[] = $value;
    }
    
    $datasets[] = [
        'label' => 'Niveles de Alcalinidad (mg/L)',
        'data' => $alkalinity_mg_l_data,
        'type' => 'line',
        'backgroundColor' => $colors['alkalinity_mg_l'][0],
        'borderColor' => $colors['alkalinity_mg_l'][1],
        'borderWidth' => 3,
        'fill' => false,
        'tension' => 0.4,
        'pointBackgroundColor' => $colors['alkalinity_mg_l'][1],
    ...
    foreach ($all_months as $index => $month) {
        ...
        if (isset($months_data[$month])) {
            ...
            $month_tooltip['expenses_and_alcalinidad'] = [
                ...
                'avg_alkalinity_mg_l' => $data['avg_alkalinity_mg_l'] . ' mg/L'
            ];
        } else {
            $month_tooltip['expenses_and_alcalinidad'] = [
                'total_expenses' => '$0.00',
