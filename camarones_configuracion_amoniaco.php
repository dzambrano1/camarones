...
<title>Camarones Configuracion Amoniacos</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
  ...
  CONFIGURACION AMONIACOS
  </h3>
</div> 
<!-- New Entry Modal Configuracion Amoniacos -->

<!-- Add New Vacuna Amoniacos Button -->
...
<div class="modal fade" id="newEntryModal" tabindex="-1" aria-labelledby="newEntryModalLabel">
      ...
      <div class="modal-content">
          <div class="modal-header">
              <h5 class="modal-title" id="newEntryModalLabel">
                  <i class="fas fa-plus-circle me-2"></i>Configurar Nuevo Amoniaco
              </h5>
  ...
  <!-- DataTable for cac_ammonia records -->
  
<div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="amoniacosTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
              ...
              <tbody>
                  <?php
                      $amoniacoQuery = "SELECT * FROM cac_ammonia";

                      $stmt = $conn->prepare($amoniacoQuery);
                      $stmt->execute();
                      $amoniacoData = $stmt->fetchAll(PDO::FETCH_ASSOC);

                      if (empty($amoniacoData)) {
                          echo "<tr><td colspan='5' class='text-center'>No hay registros disponibles</td></tr>";
                      } else {
                          foreach ($amoniacoData as $row) {
                              echo "<tr>";
                              
                              // Column 0: Actions
                              echo '<td class="text-center">';
                              echo '    <div class="btn-group" role="group">';
                              echo '        <button class="btn btn-warning btn-sm text-center edit-amoniaco" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '" 
                                              data-vacuna="' . htmlspecialchars($row['cac_ammonia_vacuna'] ?? '') . '" 
                                              data-dosis="' . htmlspecialchars($row['cac_ammonia_dosis'] ?? '') . '" 
                                              data-costo="' . htmlspecialchars($row['cac_ammonia_costo'] ?? '') . '" 
                                              data-vigencia="' . htmlspecialchars($row['cac_ammonia_vigencia'] ?? '') . '"
                                              title="Editar Configuracion Vacuna Amoniacos">
                                              <i class="fas fa-edit"></i>
                                          </button>';
                              echo '        <button class="btn btn-danger btn-sm delete-amoniaco" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                              title="Eliminar Configuracion Producto Amoniacos">
                              ...
                              // Column 1: Vacuna
                              echo "<td class='text-center'>" . htmlspecialchars($row['cac_ammonia_vacuna'] ?? '') . "</td>";
                              
                              // Column 2: Dosis
                              echo "<td class='text-center'>" . htmlspecialchars($row['cac_ammonia_dosis'] ?? 'N/A') . "</td>";
                              
                              // Column 3: Costo
                              echo "<td class='text-center'>" . htmlspecialchars($row['cac_ammonia_costo'] ?? 'N/A') . "</td>";
                              
                              // Column 4: Vigencia
                              echo "<td class='text-center'>" . htmlspecialchars($row['cac_ammonia_vigencia'] ?? 'N/A') . "</td>";
...
<!-- Initialize DataTable for VH amoniaco -->
<script>
$(document).ready(function() {
    $('#amoniacoTable').DataTable({
...
$(document).ready(function() {
    ...
    $('#saveNewAmoniacos').click(function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: 'process_configuracion_amoniaco.php',
                    type: 'POST',
                    data: {
                        action: 'insert',
                    ...
                    success: function(response) {
                        ...
                        Swal.fire({
                            ...
                            text: 'El registro de amoniaco ha sido guardado correctamente',
                            icon: 'success',
                            confirmButtonColor: '#28a745'
                        }).then(() => {
    ...
    $('.edit-amoniaco').click(function() {
        ...
        $('#saveEditAmoniacos').click(function() {
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar la configuracion de amoniaco?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    $('.delete-amoniaco').click(function() {
        var id = $(this).data('id');
        
        // Confirm before deleting using SweetAlert2
        Swal.fire({
            title: '¿Eliminar registro?',
            text: `¿Está seguro de que desea eliminar la configuracion de amoniaco? Esta acción no se puede deshacer.`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc3545',
    ...
    $(document).on('click', '.register-new-amoniaco-btn', function() { 
