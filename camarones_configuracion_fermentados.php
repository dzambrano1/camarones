...
<title>Camarones PLAN ALIMENTO FERMENTADOS</title>
<!-- Link to the Favicon -->
                ...
                <div class="col-md-4 mb-3">
                    <div class="card border-info h-100">
                        <div class="card-body p-3">
                            ...
                            <p class="card-text small text-muted">
                                Mezcla de fermentados de soya, trigo, arroz, pescado, y subproductos vegetales según disponibilidad local.
                            </p>
                        </div>
                    </div>
                </div>
...
<div class="container text-center">
  ...
  CONFIGURACION FERMENTADOS CAMARONES
  </h3>
</div> 
...
<div class="modal fade" id="newEntryModal" tabindex="-1" aria-labelledby="newEntryModalLabel">
      ...
      <div class="modal-content">
          <div class="modal-header">
              <h5 class="modal-title" id="newEntryModalLabel">
                  <i class="fas fa-plus-circle me-2"></i>Configurar Nuevo Fermentado Camarones
              </h5>
                  ...
                  <div class="mb-4">
                      <div class="input-group">
                          <span class="input-group-text">
                              ...
                              <label for="new_fermentados" class="form-label">Fermentado</label>
                            <input type="text" class="form-control" id="new_fermentados" name="fermentados" required>
                          </span>
                      </div>
                  </div>
                  <div class="mb-4">
  ...
  <!-- DataTable for cac_fermentados records -->
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="fermentadosTable" class="table table-striped table-bordered">
              ...
              <tbody>
                  <?php
                      $fermentadosQuery = "SELECT * FROM cac_fermentados";

                      $stmt = $conn->prepare($fermentadosQuery);
                      $stmt->execute();
                      $fermentadossData = $stmt->fetchAll(PDO::FETCH_ASSOC);

                                             if (empty($fermentadossData)) {
                           echo "<tr><td colspan='5' class='text-center'>No hay registros disponibles</td></tr>";
                       } else {
                          foreach ($fermentadossData as $row) {
                              echo "<tr>";
                              ...
                              echo '        <button class="btn btn-warning btn-sm edit-fermentados" 
                                              ...
                                              data-fermentados="' . htmlspecialchars($row['cac_fermentados_nombre'] ?? '') . '" 
                                              data-etapa="' . htmlspecialchars($row['cac_fermentados_etapa'] ?? '') . '"
                                              data-costo="' . htmlspecialchars($row['cac_fermentados_costo'] ?? '') . '" 
                                              data-vigencia="' . htmlspecialchars($row['cac_fermentados_vigencia'] ?? '') . '"
                              ...
                              // Column 1: Vacuna
                              echo "<td>" . htmlspecialchars($row['cac_fermentados_nombre'] ?? '') . "</td>";
                              // Columna 2: Etapa
                              echo "<td>" . htmlspecialchars($row['cac_fermentados_etapa'] ?? '') . "</td>";
                              // Column 4: Costo
                              echo "<td>" . htmlspecialchars($row['cac_fermentados_costo'] ?? 'N/A') . "</td>";
                              // Column 5: Vigencia
                              echo "<td>" . htmlspecialchars($row['cac_fermentados_vigencia'] ?? 'N/A') . "</td>";
                              echo "</tr>";
                          }
                      }
                  ?>
...
<!-- Initialize DataTable for fermentadosTable -->
<script>
...
$(document).ready(function() {
    ...
    $('#saveNewConcentrado').click(function() {
        ...
        // Get form data
        var formData = {
            fermentados: $('#new_fermentados').val(),
            etapa: $('#new_etapa').val(),
            costo: $('#new_costo').val(),
        ...
        Swal.fire({
            ...
            text: `¿Desea registrar el alimento ${formData.fermentados} ?`,
            icon: 'question',
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: 'process_configuracion_fermentados.php',
                    type: 'POST',
                    data: {
                        action: 'create',
                        fermentados: formData.fermentados,
                    ...
                    success: function(response) {
                        ...
                        Swal.fire({
                            ...
                            text: 'El registro de fermentados ha sido guardado correctamente',
                            icon: 'success',
                            confirmButtonColor: '#28a745'
                        }).then(() => {
    ...
    $('.edit-fermentados').click(function() {
        var id = $(this).data('id');
        var fermentados = $(this).data('fermentados');
                            ...
                            <div class="mb-2">                            
                                <div class="input-group">
                                    <span class="input-group-text">
                                        ...
                                        <label for="edit_fermentados" class="form-label">Alimento</label>                                    
                                        <select class="form-select" id="edit_fermentados" name="fermentados" required>
                                            <option value="">Seleccionar</option>
                                            <?php
                                            // Fetch distinct names from the database
                                            $sql_names = "SELECT DISTINCT cac_fermentados_nombre FROM cac_fermentados ORDER BY cac_fermentados_nombre ASC";
                                            $stmt_names = $conn->prepare($sql_names);
                                            $stmt_names->execute();
                                            $names = $stmt_names->fetchAll(PDO::FETCH_ASSOC);
                                            foreach ($names as $name_row) {
                                                echo '<option value="' . htmlspecialchars($name_row['cac_fermentados_nombre']) . '">' . htmlspecialchars($name_row['cac_fermentados_nombre']) . '</option>';
                                            }
                                            ?>
                                        </select>
                                    </span>                                    
        ...
        setTimeout(function() {
            $('#edit_fermentados').val(fermentados);
            ...
            console.log('Actual values set:', {
                fermentados: $('#edit_fermentados').val(),
        ...
        $('#saveEditConcentrado').click(function() {
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar la configuracion de fermentados?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    $('.delete-fermentados').click(function() {
        ...
        Swal.fire({
            ...
            text: `¿Está seguro de que desea eliminar la configuracion de fermentados? Esta acción no se puede deshacer.`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc3545',
    ...
    $(document).on('click', '.register-new-fermentados-btn', function() { 
...
$(document).ready(function() {
    ...
    function calculatePezROI() {
        ...
        const fermentadosConsumido = alimentoTotalConsumido;
        ...
        const racionDiariaConcentrado = duracionDias > 0 ? (fermentadosConsumido / duracionDias) : 0;
        
        // Calculate break-even point for feed cost
        const costoTotalCompra = pesoInicial * precioKgInicial;
        const ingresoVenta = pesoFinal * precioKgFinal;
        const margenDisponible = ingresoVenta - costoTotalCompra;
        const precioAlimentoEquilibrio = fermentadosConsumido > 0 ? (margenDisponible / fermentadosConsumido) : 0;
        
        // Perform financial calculations (all feed is concentrate for pigs)
        const costoTotalAlimento = fermentadosConsumido * costoAlimentoKg;
        const costoTotal = costoTotalCompra + costoTotalAlimento;
        ...
        const resultsHtml = `
            <div class="calculation-steps">
                ...
                <div class="step-item mb-3">
                    ...
                    <div class="step-calculation">
                        <code>fermentados_total = alimento_total (100% fermentados para peces)</code>
                        <div class="step-result">
                            <strong>Concentrado Total:</strong> ${formatNumber(fermentadosConsumido)} kg
                        </div>
                    </div>
                </div>
                ...
                <div class="step-item mb-4 border border-success rounded p-3" style="background: linear-gradient(135deg, #d4edda 0%, #c3e6cb 100%);">
                    ...
                    <div class="mt-2">
                        <small style="color: #155724; font-weight: 500;">
                            💡 Los peces requieren alimentación 100% con fermentados balanceado para un ${tipoPez === 'Camarones' ? 'crecimiento óptimo' : 'rendimiento productivo óptimo'}.
                        </small>
                    </div>
                </div>
                ...
                <div class="step-item mb-3">
                    ...
                    <div class="step-calculation">
                        <code>precio_equilibrio = (ingreso_venta - costo_compra) ÷ fermentados_consumido</code>
                        <div class="step-result">
                            (${formatCurrency(ingresoVenta)} - ${formatCurrency(costoTotalCompra)}) ÷ ${formatNumber(fermentadosConsumido)} kg = <strong class="${precioAlimentoEquilibrio > costoAlimentoKg ? 'text-success' : 'text-danger'}">${formatCurrency(precioAlimentoEquilibrio)}/kg</strong>
                        </div>
                        <small class="text-muted">
                            ${precioAlimentoEquilibrio > costoAlimentoKg ? 
                                '✅ El precio actual del fermentados está por debajo del punto de equilibrio' : 
                                '⚠️ El precio actual del fermentados supera el punto de equilibrio'}
                        </small>
                    </div>
                </div>
                ...
                <div class="step-item mb-3">
                    ...
                    <div class="step-calculation">
                        <code>costo_total_fermentados = fermentados_consumido × costo_fermentados_kg</code>
                        <div class="step-result">
                            ${formatNumber(fermentadosConsumido)} kg × ${formatCurrency(costoAlimentoKg)}/kg = <strong>${formatCurrency(costoTotalAlimento)}</strong>
                        </div>
                    </div>
                </div>
                ...
                <div class="step-item mb-3">
                    ...
                    <div class="step-calculation">
                        <code>costo_total = costo_total_compra + costo_total_fermentados</code>
                        <div class="step-result">
                ...
                <div class="summary-section">
                    ...
                    <div class="mt-3">
                        ...
                        <small class="text-muted">
                            ...
                            En peces, el fermentados representa el 100% de la alimentación y es el principal costo de producción.
                        </small>
                    </div>
                </div>
            </div>
