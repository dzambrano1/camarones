...
try {
    ...
    // Build the SQL query to get last 12 months of data
    $sql = "SELECT 
                pond_id,
                YEAR(fecha) as year,
                ...
                DATE_FORMAT(fecha, '%Y-%m') as month_label
    ...
    $sql .= " GROUP BY pond_id, YEAR(fecha), MONTH(fecha)
              ORDER BY YEAR(fecha), MONTH(fecha), pond_id";
    
    $stmt = $conn->prepare($sql);
    $stmt->execute($params);
    $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
    // Also get all available pond IDs for the dropdown
    $pond_sql = "SELECT DISTINCT pond_id FROM water_temperature_log ORDER BY pond_id";
    ...
    foreach ($results as $row) {
        ...
        $pondData[$pondId][$monthKey] = round(floatval($row['avg_temperature']), 2);
    }
    ...
    echo json_encode([
        'success' => false,
        'message' => 'Error al obtener datos de temperatura: ' . $e->getMessage()
    ]);
}
?>
