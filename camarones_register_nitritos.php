...
// Fetch product options from cac_nitritos table
$productOptions = [];
try {
    $sql = "SELECT id, cac_nitritos_vacuna, cac_nitritos_dosis, cac_nitritos_costo FROM cac_nitritos ORDER BY cac_nitritos_vacuna ASC";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
...
<div class="container text-center">
    ...
    <!-- New Water Nitritos Entry Modal -->
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="newEntryModalLabel">
                    <i class="fas fa-plus-circle me-2"></i>Nuevo Registro Nitritos
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="newNitritosForm">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-field">
                                <label for="new_nitrite_mg_l">Nivel amoniaco (ppt)</label>
                                <div class="field-container">
                                    <i class="fas fa-water field-icon"></i>
                                    <input type="number" step="0.01" min="0" id="new_nitrite_mg_l" name="nitrite_mg_l" placeholder="Ej: 15.50" required>
                                </div>
                            </div>
                            <div class="form-field">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-field">
                                <label for="new_product">Producto Nitritos</label>
                                <div class="field-container">
                                    <i class="fas fa-syringe field-icon"></i>
                                    <select id="new_product" name="product" required>
                                        <option value="">Productos</option>
                                        <?php foreach ($productOptions as $product): ?>
                                            <option value="<?php echo htmlspecialchars($product['cac_nitritos_vacuna']); ?>">
                                                <?php echo htmlspecialchars($product['cac_nitritos_vacuna']); ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
            ...
            <div class="modal-footer btn-group">
                ...
                <button type="button" class="btn btn-success" id="saveNewNitritos">
...
<div class="container mt-4 text-center">
    <button type="button" class="btn btn-success btn-lg" data-bs-toggle="modal" data-bs-target="#newEntryModal">
        <i class="fas fa-plus me-2"></i>Nuevo Registro de Nitritos
    </button>
  </div>
  
<!-- Simple DataTable for Water Nitritos Records -->
<div class="container mt-5 mb-5">
    <div class="card shadow">
        <div class="card-header bg-info text-white">
            <h5 class="mb-0 text-center"><i class="fas fa-table me-2"></i>Registros de Nitritos</h5>
        </div>
        <div class="card-body">
            <div class="table-responsive">
                <table id="nitritosTable" class="table table-striped table-bordered">
                    <thead class="table-dark">
                        <tr>
                            <th class="text-center">ID Estanque</th>
                            <th class="text-center">Nitritos (mg/L)</th>
...
<div class="modal fade modern-modal" id="editNitritosModal" tabindex="-1" aria-labelledby="editNitritosModalLabel">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editNitritosModalLabel">
                    <i class="fas fa-edit me-2"></i>Editar Registros Nitritos
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="editNitritosForm">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-field">
                                <label for="edit_nitritos_level">Nivel Nitritos (mg/L)</label>
                                <div class="field-container">
                                    <i class="fas fa-tint field-icon"></i>
                                    <input type="number" step="0.01" min="0" id="edit_nitritos_level" name="nitrite_mg_l" placeholder="Ej: 15.50" required>
                                </div>
                            </div>
                            <div class="form-field">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-field">
                                <label for="edit_product">Producto Nitritos</label>
                                <div class="field-container">
                                    <i class="fas fa-syringe field-icon"></i>
            ...
            <div class="modal-footer btn-group">
                ...
                <button type="button" class="btn btn-success" id="saveEditNitritos">
                    <i class="fas fa-save me-1"></i>Guardar
                </button>
            </div>
        </div>
...
<!-- Water Nitritos Statistics -->
<div class="container mt-5 mb-5">
    <div class="row">
                <div class="col-md-4">
            <div class="card shadow">
                <div class="card-header bg-info text-white">
                    <h6 class="mb-0"><i class="fas fa-tint me-2"></i>Nitritos Promedio</h6>
                </div>
                <div class="card-body text-center">
                    <h4 id="avgNitritos">-- ppt</h4>
                    </div>
                    </div>
                    </div>
        <div class="col-md-4">
            <div class="card shadow">
                <div class="card-header bg-success text-white">
                    <h6 class="mb-0"><i class="fas fa-arrow-up me-2"></i>Nitritos Máxima</h6>
                </div>
                <div class="card-body text-center">
                    <h4 id="maxNitritos">-- ppt</h4>
                    </div>
                    </div>
                     </div>
        <div class="col-md-4">
            <div class="card shadow">
                <div class="card-header bg-warning text-white">
                    <h6 class="mb-0"><i class="fas fa-arrow-down me-2"></i>Nitritos Mínima</h6>
                </div>
                <div class="card-body text-center">
                    <h4 id="minNitritos">-- ppt</h4>
                    </div>
            </div>
        </div>
    </div>
...
$(document).ready(function() {
    // Load nitritos statistics
    function loadStatistics() {
        $.ajax({
            url: 'get_water_nitrite_data.php',
            type: 'GET',
            dataType: 'json',
            success: function(response) {
                if (!response.success || !response.records || response.records.length === 0) {
                    $('#avgNitritos').text('-- mg/L');
                    $('#maxNitritos').text('-- mg/L');
                    $('#minNitritos').text('-- mg/L');
                    return;
                }

                // Calculate statistics from nitrite_mg_l column
                const nitritosLevels = response.records
                    .map(item => parseFloat(item.nitrite_mg_l))
                    .filter(value => !isNaN(value) && value > 0); // Filter out invalid values
                
                if (nitritosLevels.length === 0) {
                    $('#avgNitritos').text('-- mg/L');
                    $('#maxNitritos').text('-- mg/L');
                    $('#minNitritos').text('-- mg/L');
                    return;
                ...
                const avgNitritos = nitritosLevels.reduce((a, b) => a + b, 0) / nitritosLevels.length;
                const maxNitritos = Math.max(...nitritosLevels);
                const minNitritos = Math.min(...nitritosLevels);
                
                // Update display with proper formatting
                $('#avgNitritos').text(avgNitritos.toFixed(2) + ' mg/L');
                $('#maxNitritos').text(maxNitritos.toFixed(2) + ' mg/L');
                $('#minNitritos').text(minNitritos.toFixed(2) + ' mg/L');
            },
            error: function(xhr, status, error) {
                console.error('Error loading statistics:', error);
                $('#avgNitritos').text('Error');
                $('#maxNitritos').text('Error');
                $('#minNitritos').text('Error');
            }
        });
    }
    ...
    if (typeof nitritosDataTable !== 'undefined') {
        nitritosDataTable.on('xhr.dt', function() {
            loadStatistics();
        });
    }
});
</script>
<!-- Monthly nitritos Trends Chart -->
<div class="container mt-5 mb-5">
    <div class="card shadow">
        <div class="card-header bg-primary text-white">
            <h5 class="mb-0 text-center">
                <i class="fas fa-chart-line me-2"></i>Tendencias de nitritos Mensual por Estanque
            </h5>
        </div>
        <div class="card-body">
            <!-- Filter Controls -->
            ...
            <div class="row">
                <div class="col-12">
                    <div class="chart-container" style="position: relative; height: 500px; width: 100%;">
                        <canvas id="nitritosChart" style="width: 100%; height: 100%;"></canvas>
                    </div>
                </div>
            </div>
            ...
            <div class="row mt-3">
                <div class="col-md-12">
                    <small class="text-muted">
                        ...
                        Este gráfico muestra los niveles de nitritos promedio mensuales para cada estanque.
...
<script>
// nitritos Chart Variables
let nitritosChart = null;
let nitritosChartData = {};

// Initialize nitritos Chart
function initnitritosChart() {
    const ctx = document.getElementById('nitritosChart').getContext('2d');
    
    nitritosChart = new Chart(ctx, {
        type: 'line',
        data: {
            labels: [],
            datasets: []
        ...
        options: {
            ...
            plugins: {
                title: {
                    display: true,
                    text: 'Nitritos Promedio Mensual por Estanque',
                    font: {
                        size: 16,
                        weight: 'bold'
                    }
    ...
    setTimeout(() => {
        nitritosChart.resize();
    }, 100);
}

// Load nitritos Chart Data
function loadnitritosData(tagidFilter = 'all') {
    const url = `get_monthly_nitritos_data.php${tagidFilter !== 'all' ? '?tagid=' + encodeURIComponent(tagidFilter) : ''}`;
    ...
    fetch(url)
        ...
        .then(data => {
            if (data.success) {
                ...
                nitritosChartData = data;
                
                // Update chart
                nitritosChart.data.labels = data.labels;
                nitritosChart.data.datasets = data.datasets;
                nitritosChart.update();
                ...
                console.log('nitritos chart updated successfully');
            } else {
                console.error('Error loading nitritos data:', data.message);
                Swal.fire({
                    title: 'Error',
        ...
        .catch(error => {
            console.error('Error fetching nitritos data:', error);
            Swal.fire({
                title: 'Error',
...
// Initialize nitritos chart when page loads
$(document).ready(function() {
    // Initialize chart
    initnitritosChart();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('saveNewNitritos').addEventListener('click', saveNewRecord);
    document.getElementById('saveEditNitritos').addEventListener('click', saveEditRecord);
});

// Initialize DataTable (CRUD))
function initializeDataTable() {
    nitritosDataTable = $('#nitritosTable').DataTable({
        ajax: {
            url: 'get_water_nitrite_data.php',
            type: 'GET',
        ...
        buttons: [
            ...
            },
            {
                ...
                filename: 'nitritos_' + new Date().toISOString().split('T')[0]
            },
            {
                extend: 'excel',
    ...
    $('#nitritosTable_filter input').attr('placeholder', 'Buscar en todos los campos...');
...
function editRecord(id) {
    ...
    const rowData = nitritosDataTable.rows().data().toArray().find(r => r.id == id);
    if (!rowData) return;
    ...
    document.getElementById('edit_nitritos_level').value = rowData.nitrite_mg_l;
    ...
    // Show modal
    const modal = new bootstrap.Modal(document.getElementById('editNitritosModal'));
    modal.show();
}
...
function deleteRecord(id, pondId) {
    Swal.fire({
        ...
        fetch('process_water_nitrite.php', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (data.success) {
            ...
            nitritosDataTable.ajax.reload(); // Reload DataTable
...
function saveNewRecord() {
    const formData = {
        ...
        nitrite_mg_l: document.getElementById('new_nitrite_mg_l').value,
    ...
    .then(data => {
        if (data.success) {
            ...
            document.getElementById('newNitritosForm').reset();
...
function saveEditRecord() {
    const formData = {
        ...
        nitrite_mg_l: document.getElementById('edit_nitritos_level').value,
    ...
    .then(data => {
        if (data.success) {
            ...
            const modal = bootstrap.Modal.getInstance(document.getElementById('editNitritosModal'));
            modal.hide();
            nitritosDataTable.ajax.reload(); // Reload DataTable
            $(document).trigger('tableUpdated'); // Trigger statistics update
        } else {
