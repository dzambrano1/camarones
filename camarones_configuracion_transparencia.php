...
<title>Camarones Configuracion Transparencia</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
  ...
  CONFIGURACION TRANSPARENCIA
  </h3>
</div> 
<!-- New Entry Modal Configuracion Transparencia -->

<!-- Add New Vacuna Transparencia Button -->
...
<div class="modal fade" id="newEntryModal" tabindex="-1" aria-labelledby="newEntryModalLabel">
      ...
      <div class="modal-content">
          <div class="modal-header">
              <h5 class="modal-title" id="newEntryModalLabel">
                  <i class="fas fa-plus-circle me-2"></i>Configurar Nuevo Producto Control Transparencia
              </h5>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
              <form id="newAmoniacosForm">
              <input type="hidden" id="new_id" name="id" value="">
                  <div class="mb-4">                        
                      <div class="input-group">
                          <span class="input-group-text">
                              <i class="fa-solid fa-syringe"></i>
                              <label for="new_vacuna" class="form-label">Producto</label>
          ...
          <div class="modal-footer btn-group">
              ...
              <button type="button" class="btn btn-success" id="saveNewAmoniacos">
  ...
  <!-- DataTable for cac_transparencia records -->
  
<div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="transparenciasTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
              ...
              <tbody>
                  <?php
                      $transparenciaQuery = "SELECT * FROM cac_transparencia";

                      $stmt = $conn->prepare($transparenciaQuery);
                      $stmt->execute();
                      $transparenciaData = $stmt->fetchAll(PDO::FETCH_ASSOC);

                      if (empty($transparenciaData)) {
                          echo "<tr><td colspan='5' class='text-center'>No hay registros disponibles</td></tr>";
                      } else {
                          foreach ($transparenciaData as $row) {
                              echo "<tr>";
                              
                              // Column 0: Actions
                              echo '<td class="text-center">';
                              echo '    <div class="btn-group" role="group">';
                              echo '        <button class="btn btn-warning btn-sm edit-transparencia" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '" 
                                              data-vacuna="' . htmlspecialchars($row['cac_transparencia_vacuna'] ?? '') . '" 
                                              data-dosis="' . htmlspecialchars($row['cac_transparencia_dosis'] ?? '') . '" 
                                              data-costo="' . htmlspecialchars($row['cac_transparencia_costo'] ?? '') . '" 
                                              data-vigencia="' . htmlspecialchars($row['cac_transparencia_vigencia'] ?? '') . '"
                                              title="Editar Configuracion Producto Transparencia">
                                              <i class="fas fa-edit"></i>
                                          </button>';
                              echo '        <button class="btn btn-danger btn-sm delete-transparencia" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                              title="Eliminar Configuracion Producto Transparencia">
                              ...
                              // Column 1: Vacuna
                              echo "<td class='text-center'>" . htmlspecialchars($row['cac_transparencia_vacuna'] ?? '') . "</td>";
                              
                              // Column 2: Dosis
                              echo "<td class='text-center'>" . htmlspecialchars($row['cac_transparencia_dosis'] ?? 'N/A') . "</td>";
                              
                              // Column 3: Costo
                              echo "<td class='text-center'>" . htmlspecialchars($row['cac_transparencia_costo'] ?? 'N/A') . "</td>";
                              
                              // Column 4: Vigencia
                              echo "<td class='text-center'>" . htmlspecialchars($row['cac_transparencia_vigencia'] ?? 'N/A') . "</td>";
...
<!-- Initialize DataTable for VH transparencia -->
<script>
$(document).ready(function() {
    $('#transparenciaTable').DataTable({
...
$(document).ready(function() {
    ...
    // Note: editAmoniacosModal is created dynamically later, so no need to initialize here.

    // Handle new entry form submission
    $('#saveNewAmoniacos').click(function() {
        // Validate the form
        var form = document.getElementById('newAmoniacosForm');
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: 'process_configuracion_transparencia.php',
                    type: 'POST',
                    data: {
                        action: 'insert',
                    ...
                    success: function(response) {
                        ...
                        Swal.fire({
                            ...
                            text: 'El registro de transparencia ha sido guardado correctamente',
                            icon: 'success',
                            confirmButtonColor: '#28a745'
                        }).then(() => {
    ...
    $('.edit-transparencia').click(function() {
        var id = $(this).data('id');
        ...
        // Edit Configuracion Amoniacos Modal dialog for editing
        var modalHtml = `
        <div class="modal fade" id="editAmoniacosModal" tabindex="-1" aria-labelledby="editAmoniacosModalLabel">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editAmoniacosModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Producto Control Amoniaco
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form id="editAmoniacosForm">
                            ...
                            <div class="mb-2">                            
                                <div class="input-group">
                                    <span class="input-group-text">
                                        ...
                                        <label for="edit_vacuna" class="form-label">Producto</label>                                    
                    ...
                    <div class="modal-footer btn-group">
                        ...
                        <button type="button" class="btn btn-success" id="saveEditAmoniacos">
        ...
        $('#editAmoniacosModal').remove();
        
        // Add the modal to the page
        $('body').append(modalHtml);
        
        // Show the modal
        var editModal = new bootstrap.Modal(document.getElementById('editAmoniacosModal'));
        editModal.show();
        
        // Handle save button click
        $('#saveEditAmoniacos').click(function() {
            // Create a form object to properly validate
            var form = document.getElementById('editAmoniacosForm');
            if (!form.checkValidity()) {
                form.reportValidity();
                return;
            }
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar la configuracion de transparencia?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    $('.delete-transparencia').click(function() {
        var id = $(this).data('id');
        
        // Confirm before deleting using SweetAlert2
        Swal.fire({
            title: '¿Eliminar registro?',
            text: `¿Está seguro de que desea eliminar la configuracion de transparencia? Esta acción no se puede deshacer.`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc3545',
    ...
    $(document).on('click', '.register-new-transparencia-btn', function() { 
